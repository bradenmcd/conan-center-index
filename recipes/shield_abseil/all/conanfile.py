...
class ShieldAbseilConan(ConanFile):
    name = "shield_abseil"
    description = "Shield AI hosted Abseil Common Libraries (C++) from Google"
    ...
    def validate(self):
        ...
        if (
            self.options.shared
            and is_msvc(self)
            and Version(self.version) < "20230802.1"
        ):
            # upstream tries its best to export symbols, but it's broken for the moment
            raise ConanInvalidConfiguration(
                f"{self.ref} shared not availabe for Visual Studio, please use version 20230802.1 or newer"
            )
    ...
    def generate(self):
        ...
        tc.variables["SHIELD_ABSL_ENABLE_INSTALL"] = True
        tc.variables["SHIELD_ABSL_PROPAGATE_CXX_STD"] = True
    ...
    def _patch_sources(self):
        ...
        if cross_building(self):
            toolchain_file = os.path.join(
                self.generators_folder, "conan_toolchain.cmake"
            )
    ...
    def build(self):
        ...
        abi_file.replace_in_options_file(
            os.path.join(self.source_folder, "shield_absl", "base", "options.h")
        )
        cmake.build()
    ...
    def package(self):
        ...
        absl_targets_file = os.path.join(
            cmake_folder, "shield_absl", "shield_abslTargets.cmake"
        )
    ...
    def _load_components_from_cmake_target_file(self, absl_target_file_path):
        ...
        for cmake_function_name, cmake_function_args in cmake_functions:
            ...
            potential_lib_name = "shield_absl_" + cmake_target_nonamespace

            components.setdefault(
                potential_lib_name, {"cmake_target": cmake_target_nonamespace}
            )
            ...
            if cmake_function_name == "add_library":
                ...
                if cmake_imported_target_type in ["STATIC", "SHARED"]:
                    components[potential_lib_name]["libs"] = (
                        [potential_lib_name]
                        if cmake_target_nonamespace != "shield_abseil_dll"
                        else ["shield_abseil_dll"]
                    )
                ...
                for target_property in target_properties:
                    ...
                    if property_type == "INTERFACE_LINK_LIBRARIES":
                        ...
                        for dependency in values_list:
                            if dependency.startswith("shield_absl::"):  # abseil targets
                                components[potential_lib_name].setdefault(
                                    "requires", []
                                ).append(
                                    dependency.replace("shield_absl::", "shield_absl_")
                                )
                        ...
                        for definition in values_list:
                            if (
                                definition
                                == r"\$<\$<PLATFORM_ID:AIX>:_LINUX_SOURCE_COMPAT>"
                            ):
                                if self.settings.os == "AIX":
                                    components[potential_lib_name].setdefault(
                                        "defines", []
                                    ).append("_LINUX_SOURCE_COMPAT")
                            else:
                                components[potential_lib_name].setdefault(
                                    "defines", []
                                ).append(definition)
    ...
    def _create_cxx_std_module_file(self, output_file, components):
        content = ""
        cxx_std_required = _ABIFile(
            self, os.path.join(self.build_folder, "abi.h")
        ).cxx_std()
        for _, values in components.items():
            cmake_target = values["cmake_target"]
            content += f"target_compile_features(shield_absl::{cmake_target} INTERFACE cxx_std_{cxx_std_required})\n"
        save(self, output_file, content)

    @property
    def _cxx_std_module_filepath(self):
        return os.path.join(
            self.package_folder, "lib", "cmake", "conan_trick", "cxx_std.cmake"
        )

    def package_info(self):
        self.cpp_info.set_property("cmake_file_name", "shield_absl")

        components_json_file = load(self, self._components_helper_filepath)
        shield_abseil_components = json.loads(components_json_file)
        for pkgconfig_name, values in shield_abseil_components.items():
            cmake_target = values["cmake_target"]
            self.cpp_info.components[pkgconfig_name].set_property(
                "cmake_target_name", "shield_absl::{}".format(cmake_target)
            )
            self.cpp_info.components[pkgconfig_name].set_property(
                "pkg_config_name", pkgconfig_name
            )
            ...
            self.cpp_info.components[pkgconfig_name].system_libs = values.get(
                "system_libs", []
            )
            self.cpp_info.components[pkgconfig_name].frameworks = values.get(
                "frameworks", []
            )
            self.cpp_info.components[pkgconfig_name].requires = values.get(
                "requires", []
            )
        ...
        self.cpp_info.names["cmake_find_package"] = "shield_absl"
        self.cpp_info.names["cmake_find_package_multi"] = "shield_absl"

        self.cpp_info.set_property(
            "cmake_build_modules", [self._cxx_std_module_filepath]
        )
        self.cpp_info.components["shield_absl_config"].build_modules[
            "cmake_find_package"
    ...
    def replace_in_options_file(self, options_filepath):
        for name, value in self.abi.items():
            replace_in_file(
                self.conanfile,
                options_filepath,
                "#define SHIELD_ABSL_OPTION_{} 2".format(name),
                "#define SHIELD_ABSL_OPTION_{} {}".format(name, value),
            )
